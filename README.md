...
A GitHub Actions workflow (`Merge Safety Guard`) now validates the whole repository state by failing on merge markers in source/config files and strict-JSON parsing for every `.json` file.
